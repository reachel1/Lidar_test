...
class MessageSync {
   public:
    ...
    void ProcessIMU(IMUPtr imu) {
        ...
        // std::cout<<std::fixed<<std::setprecision(6)<<"imu msg time: "<<timestamp<<std::endl;
    ...
    void ProcessCloud(const sensor_msgs::PointCloud2::ConstPtr &msg) {
        std::cout<<std::fixed<<std::setprecision(6)<<"lidar msg time: "<<msg->header.stamp.toSec()<<std::endl;
   ...
   private:
    ...
    double last_timestamp_lidar_ = -1.0;               // 最近lidar时间
