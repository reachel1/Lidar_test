...
#include "loosely_lio/loosely_lio.h"
...
void LooselyLIO::Undistort() {
    ...
    if (options_.save_motion_undistortion_pcd_) {
        // sad::SaveCloudToFile("/home/crq/workspace_HY/catkin_openhy_lidar/openhy_lidar/data/undistort_cloud/before_undist"+std::to_string(num_undistort_pcd)+".pcd", *cloud);
    }
    ...
    if (options_.save_motion_undistortion_pcd_) {
        // sad::SaveCloudToFile("/home/crq/workspace_HY/catkin_openhy_lidar/openhy_lidar/data/undistort_cloud/after_undist"+std::to_string(num_undistort_pcd)+".pcd", *cloud);
    }
    num_undistort_pcd++;
}
...
void LooselyLIO::Align() {
    ...
    if (flg_first_scan_) {
        ...
        inc_ndt_lo_->Savetime(measures_.lidar_begin_time_);
    ...
    // eskf_.ObserveSE3(pose_of_lo_, 1e-6, 1e-6);
    // inc_ndt_lo_->viewpcl_map(current_scan_filter, eskf_.GetNominalSE3());
