...
void CloudConvert::RobosenseHandler(const sensor_msgs::PointCloud2::ConstPtr &msg) {
    ...
    for (int i = 0; i < pl_orig.points.size(); i++) {
        ...
        added_pt.z = pl_orig.points[i].z;
        added_pt.intensity = pl_orig.points[i].intensity;
        added_pt.time = pl_orig.points[i].timestamp;  // curvature unit: ms

        cloud_out_.points.push_back(added_pt);
    }
    cloud_out_.is_dense = false;
    std::cout<<std::fixed<<std::setprecision(6)<<"cloud back time: "<<cloud_out_.points.back().time<<std::endl;
}
...
void CloudConvert::LoadFromYAML(const std::string &yaml_file) {
    ...
    if (lidar_type == 1) {
        ...
        lidar_type_ = LidarType::ROBOSENSE32;
