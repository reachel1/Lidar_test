...
bool MessageSync::Sync() {
    ...
    if (!lidar_pushed_) {
        ...
        lidar_end_time_ = measures_.lidar_->points.back().time;
        std::cout<<std::fixed<<std::setprecision(6)<<"lidar_begin_time: "<<measures_.lidar_begin_time_<<"  lidar_end_time: "<<lidar_end_time_
                 <<"  lidar back time: "<<measures_.lidar_->points.back().time<<std::endl;
        measures_.lidar_end_time_ = lidar_end_time_;
        lidar_pushed_ = true;
    }

    std::cout<<std::fixed<<std::setprecision(6)<<"lidar_end_time: "<<lidar_end_time_
                 <<"  imu_last_time: "<<last_timestamp_imu_<<std::endl;
