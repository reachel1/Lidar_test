...
class IncrementalNDTLO {
   public:
    ...
    void Savetime(double timestamp){
        time_queue.push_back(timestamp);
    }

    void viewpcl_map(CloudPtr cloud, const SE3& pose){
        CloudPtr cloud_world(new PointCloudType);
        pcl::transformPointCloud(*cloud, *cloud_world, pose.matrix().cast<float>());
        viewer_->SetPoseAndCloud(pose, cloud_world);
